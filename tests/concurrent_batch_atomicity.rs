...
use bloodstone::{BloodStone, Config};

const CONCURRENCY: usize = 32;
const N_KEYS: usize = 1024;
const LEAF_FANOUT: usize = 8;

fn batch_writer(db: BloodStone<64, LEAF_FANOUT, 128>, barrier: Arc<Barrier>, thread_number: usize) {
    barrier.wait();
...
fn concurrent_batch_atomicity() {
    let db: BloodStone<64, LEAF_FANOUT, 128> = Config {
    ...
    for thread_number in 0..CONCURRENCY {
        ...
        let jh = thread::spawn(move || batch_writer(db, barrier, thread_number));
        threads.push(jh);
    }
