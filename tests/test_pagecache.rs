...
fn basic_recovery() {
    ...
    let mut pc2 = PageCache::new(TestMaterializer, Some(path.to_owned()));
    pc2.recover(0);
    ...
    let mut pc3 = PageCache::new(TestMaterializer, Some(path.to_owned()));
    pc3.recover(0);
    ...
    let mut pc4 = PageCache::new(TestMaterializer, Some(path.to_owned()));
    pc4.recover(0);
    let res = pc4.get(id);
    assert_eq!(res, None);
    drop(pc4);
